...
def plot_results(predictions, actuals, train_losses, val_losses, metrics=None):
    ...
    plt.title('Predictions vs Actuals (Sample Period)')
    ...
    plt.title('Predictions vs Actuals (Full Test Set)')
    plt.xlabel('Time Step')
    plt.ylabel('Power Load')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('full_predictions.png', dpi=300)
    plt.close()

def main():
    print("Loading the dataset...")
    df = pd.read_csv('Consum 20232024 National.csv')
    ...
    plot_results(predictions, actuals, train_losses, val_losses, metrics)
